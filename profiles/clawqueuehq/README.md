...
python3 scripts/install_launchd.py --repo "$HOME/ClawQueue" --profile clawqueuehq
```
...
Keep the tracked policy portable. Put shared ClawQueue-owned non-secret defaults in `profiles/clawqueuehq/config/workflow_policy.md`; put each operator's local paths, agent IDs, assignee, notification targets, credentials, and private overrides in ignored `profiles/clawqueuehq/config/clawqueue.private.json`.

For shared profiles, run CQ with `--profile clawqueuehq` rather than `CLAWQUEUE_POLICY_FILE`; profile selection resolves `modes/`, `agents/`, and the ignored private config from each user's local checkout.
...
For agent routing, keep the tracked policy role-based (`ceo`, `cto`, `cmo`, etc.) and map those roles to your local OpenClaw runtime ids in ignored `profiles/clawqueuehq/config/clawqueue.private.json`. For example, your local `cmo` role can point to `manobot-cmo` without baking that runtime-specific name into CQ’s shared semantics.
