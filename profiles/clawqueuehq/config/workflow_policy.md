...
- Private GitHub Project node IDs, status option IDs, assignee names, notification credentials, local agent IDs, and machine-specific paths belong in ignored `profiles/clawqueuehq/config/clawqueue.private.json`, `profiles/clawqueuehq/secrets/`, or environment variables.
- Shared deployments should run with `--profile clawqueuehq`, not only `CLAWQUEUE_POLICY_FILE`, so CQ resolves profile-relative modes/agents and each operator's ignored private config from their own checkout.
